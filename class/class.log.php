<?php
  ...
  class Log
  {
      ...
      private function check_file()
      {
          try
          {
              if (file_exists(LOG_DIR . LOG_FILE))
              {
                  ...
                  {
                      $success = rename(LOG_DIR . LOG_FILE, LOG_DIR . LOG_FILE."_".date("Y-m-d H:i:s"));
                      if (!$success)
                      {
      ...
      public function write_log($string, $origen='Callcenter',$type = 0, $level = 1)
      {

          try
          {
              $formatted = sprintf(LOG_TMPLT, date('Y-m-d H:i:s'), $origen, $_SERVER['REMOTE_ADDR'], $string . "\n");
      ...
      {
          global $obj_bd;
          $query = " INSERT INTO log ( lg_lt_id_log_type, text, level, alert, timestamp ) "
                  . " VALUES ( "
                  . "  " . $type . ", "
